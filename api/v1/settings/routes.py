...
def handle_operation_failure(e):
    ...
    try:
        ...
        log(os.getenv("LOGARBOR_SETTINGS_SERVICE_ID"), "critical", f"failed db operation at: {request.path} and error: {str(e)}", os.getenv("LOGARBOR_SUPPORT_TEAM_ACCESS_TOKEN"))
...
def handle_operation_failure_pymongo(e):
    ...
    try:
        ...
        log(os.getenv("LOGARBOR_SETTINGS_SERVICE_ID"), "critical", f"failed db operation at: {request.path} and error: {str(e)} because of a pymongo error", os.getenv("LOGARBOR_SUPPORT_TEAM_ACCESS_TOKEN"))
    except Exception as loge:
...
def handle_operation_failure_exception(e):
    ...
    try:
        ...
        log(os.getenv("LOGARBOR_SETTINGS_SERVICE_ID"), "critical", f"failed at: {request.path} and error: {str(e)}", os.getenv("LOGARBOR_SUPPORT_TEAM_ACCESS_TOKEN"))
    except Exception as loge:
...
def data_validation():
    ...
    if request.method in allowed_http_methods and request.path != "/api/v1/settings/account":
        ...
        if "error" in data:
            log(os.getenv("LOGARBOR_SETTINGS_SERVICE_ID"), "error", f"failed api validation on: {request.path} with error: {data}", os.getenv("LOGARBOR_SUPPORT_TEAM_ACCESS_TOKEN"))
...
def settings_page():
    ...
    if not check["ok"]:
        ...
        log(os.getenv("LOGARBOR_SETTINGS_SERVICE_ID"), "warning", f"ui route was accessed with non ui blueprint: {request.path}", os.getenv("LOGARBOR_SUPPORT_TEAM_ACCESS_TOKEN"))
...
def settings_info():
    ...
    if not check["ok"]:
        ...
        log(os.getenv("LOGARBOR_SETTINGS_SERVICE_ID"), "warning", f"api route was accessed with non api blueprint: {request.path}", os.getenv("LOGARBOR_SUPPORT_TEAM_ACCESS_TOKEN"))
